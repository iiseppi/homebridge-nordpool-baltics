...
exports.omie_convertDataStructure = omie_convertDataStructure;
exports.omie_getNordpoolData = omie_getNordpoolData;
...
async function fetchOmieFile(log, date) {
    const url = buildOmieUrl(date);
    try {
        const response = await axios_1.default.get(url, { timeout: 10000, responseType: 'text' });
...
function omie_convertDataStructure(csvText, area, config) {
    var _a;
    const decimalPrecision = (_a = config.decimalPrecision) !== null && _a !== void 0 ? _a : 1;
    const results = [];
    for (const line of csvText.split('\n')) {
        const trimmed = line.trim();
        if (!trimmed || trimmed === '*' || trimmed.startsWith('MARGINAL'))
            continue;
        ...
        if (isNaN(year) || isNaN(month) || isNaN(day) || isNaN(quarter))
            continue;
        ...
        const dt = luxon_1.DateTime.fromObject({ year, month, day, hour: 0, minute: 0, second: 0 }, { zone: OMIE_MARKET_TIMEZONE }).plus({ minutes: offsetMinutes })
            .setZone((0, settings_1.defaultAreaTimezone)(config));
        results.push({
...
async function omie_getNordpoolData(log, config) {
    ...
    const today = luxon_1.DateTime.now().setZone(OMIE_MARKET_TIMEZONE);
...
//# sourceMappingURL=funcs_OMIE.js.map
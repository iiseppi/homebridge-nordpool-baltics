...
class NordpoolPlatformAccessory {
    constructor(platform, accessory, api) {
        ...
        this.platform.log.debug(`[${this.deviceConfig.name}] Loaded device config: ${JSON.stringify(this.deviceConfig)}`);
    ...
    async calculateCheapestStatus(allPrices, todayKey, tomorrowKey) {
        const timezone = (0, settings_1.defaultAreaTimezone)(this.platform.config);
        const now = luxon_1.DateTime.local().setZone(timezone).startOf('hour');
        const { rangeStart, rangeEnd, cheapestHours } = this.deviceConfig;
        this.platform.log.debug(`[${this.deviceConfig.name}] Calculating with rangeStart=${rangeStart}, ` +
    ...
    calculateSameDayStatus(allPrices, todayKey, now) {
        const { rangeStart, rangeEnd, cheapestHours } = this.deviceConfig;
        const targetHours = allPrices.filter(p => p.dateKey === todayKey &&
            p.hour >= rangeStart &&
            p.hour <= rangeEnd);
        if (targetHours.length === 0) {
            this.platform.log.warn(`[${this.deviceConfig.name}] No price data for range ` +
    ...
    async calculateOvernightStatus(allPrices, todayKey, tomorrowKey, now) {
        const { rangeStart, rangeEnd, cheapestHours } = this.deviceConfig;
        ...
        if (isMorningPartOfActiveOvernight) {
            ...
            const activeSchedule = await this.pricesCache.get(activeScheduleKey);
            if (!activeSchedule) {
                this.platform.log.warn(`[${this.deviceConfig.name}] No precomputed overnight schedule for active window ` +
        ...
        let upcomingSchedule = await this.pricesCache.get(upcomingScheduleKey);
        if (!upcomingSchedule) {
            const targetHours = allPrices.filter(p => (p.dateKey === todayKey && p.hour >= rangeStart) ||
            ...
            this.platform.log.info(`[${this.deviceConfig.name}] Created overnight schedule ` +
    ...
    setHomeKitAndHistoryState(isCurrentlyOn) {
        /*
        ...
        this.platform.log.info(`[${this.deviceConfig.name}] Status update: ` +
            `${isCurrentlyOn ? 'ON (Cheap)' : 'OFF (Expensive)'} ` +
            '(Pulsed to ensure automation trigger)');
    }
    selectCheapestHours(targetHours, cheapestHours) {
        return [...targetHours]
